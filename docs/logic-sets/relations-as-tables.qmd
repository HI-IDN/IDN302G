...
við upp `character.character_id = 1` og `faction.faction_id = 100`. Þá fáum við: Logi er í
Uppreisnarmönnum.
...
Táknið $\subseteq$ má lesa sem „er hlutmengi af“. Línan segir því að `member_of` sé hlutmengi af
öllum mögulegum pörum úr `character` og `faction`. Í okkar dæmi er `member_of` safn þeirra para sem
eru raunverulega skráð: fyrra stakið kemur úr hópi persóna og seinna stakið úr hópi liða.
...
auðkennunum sjáum við: Finnur birtist í `Mátturinn vaknar`.
...
Með orðum: `appears_in` er hlutmengi af öllum mögulegum pörum úr `character` og `movie`. Taflan
geymir bara þau pör sem eru skráð í dæminu.
...
liði. Finnur getur til dæmis tengst bæði Frumreglunni og Andspyrnunni. Boba Fett tengist hins vegar
hausaveiðurum og er hvorki skráður í Uppreisnarmenn né í Keisaradæmið. Ein persóna getur líka
tengst mörgum röðum í `appears_in`, því sama persóna getur birst í mörgum myndum.

## Af hverju margar litlar töflur?

Ef við skrifum nafnið `Uppreisnarmenn` í hverja einustu röð þar sem liðið kemur fyrir, þurfum við að
...
::: {.callout-note title="Lesið töflurnar sem staðreyndir"}
Svarið með orðum áður en þið hugsið um SQL:

1. Hvaða vensl táknar `member_of`?
2. Hvaða vensl táknar `appears_in`?
3. Af hverju er betra að geyma `Uppreisnarmenn` einu sinni í `faction` frekar en að skrifa nafnið í
   hverja röð í `member_of`?
4. Af hverju getur sama persóna haft margar raðir í `member_of`?
5. Hvernig mynduð þið orða skilyrðið „persónur sem eru hvorki í Uppreisnarmönnum né Keisaradæminu“
   án þess að skrifa SQL?
:::
