...
> sjúklinga. Lesið [notkun á _SQLite_ skipanaskel](database_interaction.qmd#notkun-á-skipanaskel)
...
Hvað þýðir þetta allt saman og hvernig tengist þetta
[rökfræði og mengjum](../logic-sets/index.qmd)? Þegar við skrifum `WHERE` skilyrði erum við að
þýða orðadæmi yfir í próf sem hver röð þarf að standast.

SQL skipunin `SELECT * FROM A WHERE ...;` þýðir að velja raðir úr töflunni $A$ sem uppfylla
skilyrðið í `WHERE`. Í mengjahugsun köllum við eina röð $x$ og skilyrðið $P(x)$. Þá er
niðurstaðan:
...
sem þýðir „mengi þeirra staka $x$ úr $A$ sem uppfylla yrðinguna $P(x)$.“

Hér er sama brúin í einföldu máli:

- **Tafla** er hópur raða sem við getum spurt um.
- **Röð** er eitt tilvik í hópnum.
- **Skilyrði** er yrðing sem er sönn eða ósönn fyrir hverja röð.
- **`AND`, `OR` og `NOT`** fylgja sömu hugsun og `og`, `eða` og `ekki` í sanntöflum.

Þess vegna er gott að byrja á orðadæminu. Ef þið getið sagt „ég vil raðir þar sem ...“ er stutt
skref yfir í `WHERE`.
